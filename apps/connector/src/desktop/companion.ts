...
export const buildPowerShellContinueScript = (
  ...
  return [
    ...
    "  return $bestButton",
    "}",
    "function Normalize-CodexRelayLookup([string]$value) {",
    "  if ([string]::IsNullOrWhiteSpace($value)) { return '' }",
    "  return (($value.ToLowerInvariant() -replace '\\s+', ' ').Trim())",
    "}",
    "function Get-CodexRelayThreadSignature([string]$value) {",
    "  if ([string]::IsNullOrWhiteSpace($value)) { return '' }",
    "  $trimmed = $value.Trim()",
    "  $trimmed = $trimmed -replace '\\s*(?:\\+\\s*\\d+\\s*-\\s*\\d+\\s*)?\\d+[smhdwy]\\s*$', ''",
    "  $trimmed = $trimmed -replace '\\s*\\+\\-?\\d+[smhdwy]\\s*$', ''",
    "  $trimmed = $trimmed -replace '\\s+', ' '",
    "  if ($trimmed.Length -gt 120) { $trimmed = $trimmed.Substring(0, 120) }",
    "  return Normalize-CodexRelayLookup $trimmed",
    "}",
    "function Get-CodexRelayActiveViewState($root) {",
    "  if ($null -eq $root) { return $null }",
    "  $docCondition = New-Object System.Windows.Automation.PropertyCondition(",
    "    [System.Windows.Automation.AutomationElement]::ControlTypeProperty,",
    "    [System.Windows.Automation.ControlType]::Document",
    "  )",
    "  $doc = $root.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $docCondition)",
    ...
    "    if ($null -eq $threadText -and $element.Current.ControlType -eq [System.Windows.Automation.ControlType]::Text) {",
    "      $trimmedText = $name.Trim()",
    "      if ($trimmedText.Length -ge 20 -and $trimmedText -ne 'Threads') {",
    "        $threadText = $trimmedText",
    "        continue",
    "      }",
    "    }",
    "    if ($null -eq $project -and $element.Current.ControlType -eq [System.Windows.Automation.ControlType]::Button) {",
    "      if ($name -in @('Hide sidebar','Thread actions','Set up a run action','Open','Secondary action','Toggle terminal','Toggle diff panel','Open in Popout Window','New thread','Automations','Skills','Add new project','Filter sidebar threads','Collapse folder','Archive thread','Pin thread')) { continue }",
    "      if ($name -like 'Project actions for *' -or $name -like 'Start new thread in *' -or $name -like 'Archive thread*') { continue }",
    "      $project = $name.Trim()",
    "    }",
    "    if ($null -ne $project -and $null -ne $threadText) { break }",
    "  }",
    "  return @{ Project = $project; ThreadText = $threadText }",
    "}",
    "function Test-CodexRelayActiveView($root, [string]$projectLabel, [string]$threadName) {",
    "  $state = Get-CodexRelayActiveViewState $root",
    "  if ($null -eq $state) { return $false }",
    "  $normalizedProject = Normalize-CodexRelayLookup $projectLabel",
    "  if (-not [string]::IsNullOrWhiteSpace($normalizedProject)) {",
    "    if ((Normalize-CodexRelayLookup $state.Project) -ne $normalizedProject) { return $false }",
    "  }",
    "  $signature = Get-CodexRelayThreadSignature $threadName",
    "  if ([string]::IsNullOrWhiteSpace($signature)) { return $true }",
    "  $activeThreadText = Normalize-CodexRelayLookup $state.ThreadText",
    "  if ([string]::IsNullOrWhiteSpace($activeThreadText)) { return $false }",
    "  if ($activeThreadText.StartsWith($signature)) { return $true }",
    "  $prefixLength = [Math]::Min($signature.Length, 48)",
    "  if ($prefixLength -le 0) { return $false }",
    "  return $activeThreadText.Contains($signature.Substring(0, $prefixLength))",
    "}",
    "function Open-CodexRelayThreadItem($root, [string]$projectLabel, $threadItem) {",
    "  if ($null -eq $threadItem) { return $false }",
    "  $threadName = $threadItem.Current.Name",
    "  if (Test-CodexRelayActiveView $root $projectLabel $threadName) { return $true }",
    "  try {",
    ...
    "    if (Open-CodexRelayThreadItem $root $projectLabel $candidate) {",
    ...
    "      if (Open-CodexRelayThreadItem $root '' $threadItem) {",
    "        Start-Sleep -Milliseconds 400",
    "        return $name",
    "      }",
    "    }",
